...
impl HttpServer {
    ...
    pub fn requests(&mut self) -> Result<Vec<ServerRequest>> {
        ...
        let mut events = vec![epoll::EpollEvent::default(); MAX_CONNECTIONS + 2];
        ...
        // the array is still `MAX_CONNECTIONS + 2`, so we discard empty elements
